...
function adaptiveQ(topic,problem,correct,wrong,explanation,structureKey,valueKey,subskill,tier){
 const opts=shuffle([{html:correct,correct:true},...wrong.slice(0,3).map(x=>({html:x,correct:false}))]);
...
function makeAlgebraV(tier,forced){const sub=forced||pickSubskill('algebra');
 if(sub==='Simplifying expressions'){
  if(tier<=2){const a=rnd(2,9),b=rnd(2,9),c=rnd(1,12);return adaptiveQ(sub,`${a}x + ${b}x − ${c}`,`${a}x + ${b}x − ${c}<br>= ${a+b}x − ${c}`,[`${a+b-c}x`,`= ${a*b}x − ${c}`,`= ${a+b}x`],'Only like x-terms combine; the constant remains separate.','ALG-SIMP-LIKE',`ALG-SIMP-${a}-${b}-${c}`,sub,tier)}
  if(tier===3){const a=rnd(2,7),b=rnd(2,6),c=rnd(1,9),d=rnd(1,8);return adaptiveQ(sub,`${a}(${b}x − ${c}) + ${d}x`,`${a*b}x − ${a*c} + ${d}x<br>= ${a*b+d}x − ${a*c}`,[`${a*b}x − ${c+d}`,`${a+b+d}x − ${a*c}`,`${a*b+d}x − ${c}`],'Expand every term in the bracket before collecting like terms.','ALG-SIMP-BRACKET',`ALG-BR-${a}-${b}-${c}-${d}`,sub,tier)}
 ...
 if(sub==='Fractions'){
  const x=rnd(2,12),d=rnd(2,7),a=rnd(1,6),rhs=rnd(2,14),b=rhs*d-a*x;return adaptiveQ('Equations with fractions',`(${a}x ${signed(b)})/${d} = ${rhs}`,`${a}x ${signed(b)} = ${rhs*d}<br>${a}x = ${rhs*d-b}<br>x = ${x}`,[`${a}x ${signed(b)} = ${rhs+d}<br>x = ${fmt((rhs+d-b)/a)}`,`${a}x = ${rhs*d+b}<br>x = ${fmt((rhs*d+b)/a)}`,`x = ${rhs*d-b}`],'Multiply the whole equation by the denominator, then isolate x.','ALG-EQ-FRAC',`ALG-EQF-${a}-${b}-${d}-${x}-${rhs}`,sub,tier)
 }
...
function makeGeometryV(tier,forced){const sub=forced||pickSubskill('geometry');
 ...
 const solid=tier<=2?'cuboid':(Math.random()<.5?'cylinder':'cuboid'),surface=tier>=3&&Math.random()<.45;if(solid==='cuboid'){const l=rnd(3,12),w=rnd(2,10),h=rnd(2,11);if(surface)return adaptiveQ(sub,`Cuboid: ${l} cm × ${w} cm × ${h} cm. Find total surface area.`,`SA = 2(lw + lh + wh)<br>= 2(${l*w} + ${l*h} + ${w*h})<br>= ${2*(l*w+l*h+w*h)} cm²`,[`SA = lwh = ${l*w*h} cm²`,`SA = ${l+w+h} cm²`,`SA = 2(l+w+h) = ${2*(l+w+h)} cm²`],'A cuboid has three pairs of congruent rectangular faces.','GEO-SA-CUB',`GEO-SC-${l}-${w}-${h}`,sub,tier);return adaptiveQ(sub,`Cuboid: ${l} cm × ${w} cm × ${h} cm. Find volume.`,`V = lwh<br>= ${l}×${w}×${h}<br>= ${l*w*h} cm³`,[`V = 2(lw+lh+wh) = ${2*(l*w+l*h+w*h)} cm³`,`V = ${l+w+h} cm³`,`V = ${l*w} cm³`],'Volume is the product of the three perpendicular dimensions.','GEO-VOL-CUB',`GEO-VC-${l}-${w}-${h}`,sub,tier)}const r=rnd(2,8),h=rnd(4,15);if(surface)return adaptiveQ(sub,`Closed cylinder: radius ${r} cm, height ${h} cm. Find total surface area in terms of π.`,`SA = 2πr² + 2πrh<br>= ${2*r*r + 2*r*h}π cm²`,[`SA = πr²h = ${r*r*h}π`,`SA = 2πrh = ${2*r*h}π`,`SA = πr² = ${r*r}π`],'A closed cylinder has two circular ends plus the curved surface.','GEO-SA-CYL',`GEO-SCY-${r}-${h}`,sub,tier);return adaptiveQ(sub,`Cylinder: radius ${r} cm, height ${h} cm. Find volume in terms of π.`,`V = πr²h<br>= π(${r})²(${h})<br>= ${r*r*h}π cm³`,[`V = 2πrh = ${2*r*h}π`,`V = πrh = ${r*h}π`,`V = πr² = ${r*r}π`],'Cylinder volume is πr²h; do not use circumference or surface-area formulae.','GEO-VOL-CYL',`GEO-CY-${r}-${h}`,sub,tier)
}
...
function makeStatsV(tier,forced){const sub=forced||pickSubskill('statistics');
 if(sub==='Mean / median / mode'){let vals=Array.from({length:tier>=3?7:5},()=>rnd(2,18));const kind=['mean','median','mode'][rnd(0,2)];if(kind==='mean'){const sum=vals.reduce((a,b)=>a+b,0);return adaptiveQ(sub,`Data: ${vals.join(', ')}. Find the mean.`,`Mean = (${vals.join(' + ')})/${vals.length}<br>= ${fmt(sum/vals.length)}`,[`Mean = ${fmt(sum/(vals.length-1))}`,`Mean = ${median(vals)}`,`Mean = ${Math.max(...vals)-Math.min(...vals)}`],'Mean is total of all values divided by number of values.','STAT-MEAN',`STAT-M-${vals.join('-')}`,sub,tier)}if(kind==='median'){const med=median(vals);return adaptiveQ(sub,`Data: ${vals.join(', ')}. Find the median.`,`Order the data first.<br>Middle value = ${med}`,[`Median = ${fmt(vals.reduce((a,b)=>a+b,0)/vals.length)}`,`Median = ${Math.max(...vals)-Math.min(...vals)}`,`Median = ${vals[0]}`],'Median is the middle value after ordering the data.','STAT-MED',`STAT-D-${vals.join('-')}`,sub,tier)}const mode=rnd(3,15);vals=[mode,mode,...Array.from({length:Math.max(3,vals.length-2)},()=>{let v=rnd(2,18);return v===mode?v+1:v})];shuffle(vals);return adaptiveQ(sub,`Data: ${vals.join(', ')}. Find the mode.`,`The value ${mode} occurs most often.<br>Mode = ${mode}`,[`Mode = ${median(vals)}`,`Mode = ${fmt(vals.reduce((a,b)=>a+b,0)/vals.length)}`,`Mode = ${Math.max(...vals)-Math.min(...vals)}`],'Mode is the value with the greatest frequency.','STAT-MODE',`STAT-O-${vals.join('-')}`,sub,tier)}
 ...
 const displayType=tier<=1?'bar':['pie','frequency','histogram','stem','cumulative'][rnd(0,Math.min(4,tier))];
 if(displayType==='pie'){const cats=['A','B','C','D'],freq=cats.map(()=>rnd(2,15)),total=freq.reduce((a,b)=>a+b,0),idx=rnd(0,3),angle=360*freq[idx]/total;const table=`<table class="mini-data"><tr>${cats.map(c=>`<th>${c}</th>`).join('')}</tr><tr>${freq.map(f=>`<td>${f}</td>`).join('')}</tr></table>`;return adaptiveQ('Pie chart',`${table}Find the sector angle for ${cats[idx]}.`,`Angle = ${freq[idx]}/${total} × 360°<br>= ${fmt(angle)}°`,[`Angle = ${freq[idx]}×360°`,`Angle = ${total}/${freq[idx]}×360°`,`Angle = ${freq[idx]}/${total}×100°`],'Pie-chart sector angle is category frequency divided by total frequency, multiplied by 360°.','STAT-PIE',`STAT-PIE-${freq.join('-')}-${idx}`,sub,tier)}
 if(displayType==='histogram'){const width=[5,10,20][rnd(0,2)],frequency=rnd(8,40),density=frequency/width;return adaptiveQ('Histogram',`A class interval has width ${width} and frequency ${frequency}. Find its frequency density.`,`Frequency density = frequency/class width<br>= ${frequency}/${width}<br>= ${fmt(density)}`,[`= ${frequency*width}`,`= ${fmt(width/frequency)}`,`= ${frequency+width}`],'Histogram bar height is frequency density, not frequency, when class widths vary.','STAT-HIST',`STAT-H-${width}-${frequency}`,sub,tier)}
 if(displayType==='stem'){const tens=rnd(1,4),leaves=[1,3,4,6,8].map(x=>Math.min(9,x+rnd(0,1))),row=`${tens} | ${leaves.join(' ')}`,vals=leaves.map(x=>10*tens+x);return adaptiveQ('Stem-and-leaf',`Stem-and-leaf row: <b>${row}</b><br>Key: 2 | 3 means 23. Find the median of this row.`,`Values: ${vals.join(', ')}<br>Median = ${vals[2]}`,[`Median = ${tens+leaves[2]}`,`Median = ${fmt(vals.reduce((a,b)=>a+b,0)/vals.length)}`,`Median = ${vals[4]-vals[0]}`],'Convert each stem-and-leaf entry to its full value before finding the median.','STAT-STEM',`STAT-ST-${tens}-${leaves.join('-')}`,sub,tier)}
 if(displayType==='cumulative'){const q1=rnd(12,30),med=q1+rnd(6,14),q3=med+rnd(6,14);return adaptiveQ('Cumulative frequency',`From a cumulative-frequency curve: Q1 = ${q1}, median = ${med}, Q3 = ${q3}. Find the IQR.`,`IQR = Q3 − Q1<br>= ${q3} − ${q1}<br>= ${q3-q1}`,[`IQR = ${q3+q1}`,`IQR = ${med-q1}`,`IQR = ${q3-med}`],'Read Q1 and Q3 from the curve and subtract: IQR = Q3 − Q1.','STAT-CF',`STAT-CF-${q1}-${med}-${q3}`,sub,tier)}
 const cats=['A','B','C','D'],freq=cats.map(()=>rnd(2,15)),idx=rnd(0,3),table=`<table class="mini-data"><tr>${cats.map(c=>`<th>${c}</th>`).join('')}</tr><tr>${freq.map(f=>`<td>${f}</td>`).join('')}</tr></table>`;return adaptiveQ(displayType==='bar'?'Bar graph':'Frequency table',`${table}Which working correctly identifies the frequency for ${cats[idx]}?`,`Read the ${cats[idx]} column directly.<br>Frequency = ${freq[idx]}`,[`Frequency = ${freq.reduce((a,b)=>a+b,0)}`,`Frequency = ${Math.max(...freq)}`,`Frequency = ${idx+1}`],'Read the value aligned with the requested category; do not use the total or maximum unless asked.','STAT-FREQ',`STAT-F-${freq.join('-')}-${idx}`,sub,tier)
}
