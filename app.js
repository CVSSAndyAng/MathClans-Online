...
window.MathClansGame={
  ...
  refresh(){init();},
  toast(msg){toast(msg)},
  markBattleParticipantLeft(uid){if(!battle||battle.ended||!uid)return;battle.forfeitUids=battle.forfeitUids||new Set();battle.forfeitUids.add(uid);if(battle.sessionId)window.MathClansClans?.markSharedParticipantLeft?.(battle.sessionId,uid).catch?.(()=>{});effect(`⚠ ${members.find(m=>m.uid===uid)?.name||'Player'} disconnected · 0 points`);updateBattleUi();},
  joinSharedBattle(sessionId,data){joinSharedBattle(sessionId,data)},
  syncSharedBattle(sessionId,data){syncSharedBattle(sessionId,data)},
...
function renderRanking(){
 ...
 const rl=$('#rankingList');if(rl)rl.innerHTML=rows.map((r,i)=>`<div class="rank-row ${r.you?'you':''}"><b>#${r.rank||i+1}</b><strong>${r.crest||r.guardian||'🐉'} ${r.name}${r.you?'<small>Your clan</small>':''}</strong><span>${r.region||'—'}</span><b>${r.rating||1500}</b></div>`).join('');
 const players=Array.isArray(window.__mathclansPlayerRankings)?window.__mathclansPlayerRankings:[];
 const pl=$('#playerRankingList');if(pl)pl.innerHTML=players.length?players.map((r,i)=>`<div class="rank-row player-rank-row ${r.you?'you':''}"><b>#${r.rank||i+1}</b><strong>${r.avatar||'🐲'} ${r.name||'Mathling'}${r.you?'<small>You</small>':''}</strong><span>${r.schoolClass||r.yearLevel||'—'}</span><b>${r.totalSkill||0}</b></div>`).join(''):'<div class="empty-ranking">Player rankings will appear after signed-in players sync V2.0 progress.</div>';
 const ct=$('#clanRankTab'),pt=$('#playerRankTab'),cp=$('#clanRankingPanel'),pp=$('#playerRankingPanel');
 if(ct&&!ct.dataset.bound){ct.dataset.bound='1';ct.onclick=()=>{cp?.classList.remove('hidden');pp?.classList.add('hidden');ct.className='primary active';pt.className='secondary'};pt.onclick=()=>{pp?.classList.remove('hidden');cp?.classList.add('hidden');pt.className='primary active';ct.className='secondary'}}
}
...
function workedLines(html,subskill,index){
 ...
 // V1.7.1e: options should contain genuine mathematical working, not generic filler.
 // Generators below supply comparable 3-step solutions for correct and misconception paths.
...
function makeAlgebraV(tier,forced){const sub=forced||pickSubskill('algebra');
 if(sub==='Simplifying expressions'){
  ...
  const d1=[2,3,4,5,6][rnd(0,4)],d2=[3,4,5,6,8][rnd(0,4)],a=rnd(1,7),b=rnd(1,7),L=d1*d2/gcd(d1,d2),n=a*(L/d1)+b*(L/d2);return adaptiveQ(sub,`Simplify: ${a}x/${d1} + ${b}x/${d2}`,`LCM = ${L}<br>= ${a*(L/d1)}x/${L} + ${b*(L/d2)}x/${L}<br>= ${frac(n,L)}x`,[`Add numerators and denominators directly.<br>= ${(a+b)}x/${d1+d2}<br>= ${frac(a+b,d1+d2)}x`,`Use LCM ${L}, but add original numerators only.<br>= ${a+b}x/${L}<br>= ${frac(a+b,L)}x`,`Use LCM ${L}, then multiply the numerators together.<br>= ${a*b}x/${L}<br>= ${frac(a*b,L)}x`],'Use a common denominator before adding fractional algebraic terms.','ALG-SIMP-FRAC',`ALG-FR-${a}-${d1}-${b}-${d2}`,sub,tier)}
 if(sub==='Fractions'){
  const x=rnd(2,12),d=rnd(2,7),a=rnd(1,6),b=rnd(1,9),rhs=(a*x+b)/d;if(Number.isInteger(rhs))return adaptiveQ('Equations with fractions',`(${a}x + ${b})/${d} = ${rhs}`,`${a}x + ${b} = ${rhs*d}<br>${a}x = ${rhs*d-b}<br>x = ${x}`,[`${a}x + ${b} = ${rhs}+${d}<br>${a}x = ${rhs+d-b}<br>x = ${fmt((rhs+d-b)/a)}`,`${a}x + ${b} = ${rhs*d}<br>${a}x = ${rhs*d}+${b}<br>x = ${fmt((rhs*d+b)/a)}`,`${a}x + ${b} = ${rhs*d}<br>${a}x = ${rhs*d-b}<br>x = ${rhs*d-b}`],'Multiply the whole equation by the denominator, then isolate x.','ALG-EQ-FRAC',`ALG-EQF-${a}-${b}-${d}-${x}`,sub,tier);return makeAlgebraV(tier,sub)
 }
 if(sub==='Solving equations'){
  const x=rnd(-8,14),a=rnd(2,8),b=rnd(-12,12);
  if(tier<=2){const rhs=a*x+b;return adaptiveQ(sub,`${a}x ${signed(b)} = ${rhs}`,`${a}x = ${rhs} ${b>=0?'−':'+'} ${Math.abs(b)}<br>${a}x = ${a*x}<br>x = ${x}`,[`${a}x = ${rhs} ${b>=0?'+':'−'} ${Math.abs(b)}<br>${a}x = ${rhs+b}<br>x = ${fmt((rhs+b)/a)}`,`${a}x = ${rhs-b}<br>x = ${rhs-b}<br>∴ x = ${rhs-b}`,`${a}x = ${rhs}<br>x = ${fmt(rhs/a)} ${b>=0?'−':'+'} ${Math.abs(b)}<br>x = ${fmt(rhs/a-b)}`],'Undo addition/subtraction first, then divide by the coefficient.','ALG-EQ-1SIDE',`ALG-E1-${a}-${b}-${x}`,sub,tier)}
  const c=rnd(1,7),d=rnd(-10,10);let aa=a;if(aa===c)aa++;const rhs=(aa-c)*x+b-d;return adaptiveQ(sub,`${aa}x ${signed(b)} = ${c}x ${signed(d+rhs)}`,`${aa-c}x = ${d+rhs-b}<br>x = ${x}`,[`${aa+c}x = ${d+rhs-b}<br>x = (${d+rhs-b})/${aa+c}<br>x = ${fmt((d+rhs-b)/(aa+c))}`,`${aa-c}x = ${d+rhs+b}<br>x = (${d+rhs+b})/${aa-c}<br>x = ${fmt((d+rhs+b)/(aa-c))}`,`${aa-c}x = ${d+rhs-b}<br>x = ${d+rhs-b}<br>∴ x = ${d+rhs-b}`],'Move x-terms to one side and constants to the other while preserving signs.','ALG-EQ-2SIDES',`ALG-E2-${aa}-${b}-${c}-${d+rhs}-${x}`,sub,tier)}
 if(sub==='Simultaneous equations'){
  const x=rnd(-5,8),y=rnd(-5,8),a=rnd(1,5),b=rnd(1,5),c=rnd(1,5),d=rnd(1,5),r1=a*x+b*y,r2=c*x-d*y;const mult=tier>=4?rnd(2,4):1;return adaptiveQ(sub,`${a}x + ${b}y = ${r1}<br>${c}x − ${d}y = ${r2}`,`Use elimination/substitution on the complete equations.<br>Solving gives x = ${x}, y = ${y}.`,[`Add the equations without first eliminating a variable.<br>${a+c}x + ${b-d}y = ${r1+r2}<br>Then split this into x = ${fmt((r1+r2)/(a+c||1))}, y = ${fmt((r1+r2)/(b-d||1))}.`,`Equate the left sides and ignore the constants.<br>${a}x + ${b}y = ${c}x − ${d}y<br>Then use this one equation to state a single pair.`,`Set y = 0 in both equations.<br>${a}x = ${r1}, ${c}x = ${r2}<br>Take x = ${fmt(r1/a)}, y = 0.`],'Both equations must remain equivalent at every elimination or substitution step.','ALG-SIMUL',`ALG-SYS-${a}-${b}-${r1}-${c}-${d}-${r2}-${mult}`,sub,tier)}
 const a=rnd(-6,8),b=rnd(-5,7),p=rnd(2,5),q=rnd(-7,7);return adaptiveQ('Substitution',`Given a = ${a}, b = ${b}, find ${p}a² ${signed(q)} b.`,`${p}(${a})² ${signed(q)}(${b})<br>= ${p*a*a + q*b}`,[`${p}a² ${signed(q)}b<br>= (${p*a})² ${signed(q*b)}<br>= ${p*a*p*a+q*b}`,`${p}a² ${signed(q)}b<br>= ${p}(${a}×2) ${signed(q*b)}<br>= ${2*p*a+q*b}`,`${p}a² ${signed(q)}b<br>= ${p}+${a*a} ${signed(q*b)}<br>= ${p+a*a+q*b}`],'Substitute the values first, then follow order of operations.','ALG-SUB',`ALG-SUB-${a}-${b}-${p}-${q}`,sub,tier)
}
function makeGeometryV(tier,forced){const sub=forced||pickSubskill('geometry');
 if(sub==='Gradient'){const x1=rnd(-8,5),dx=rnd(2,9),m=tier>=3?([-3,-2,-1,1,2,3][rnd(0,5)]):rnd(1,5),x2=x1+dx,y1=rnd(-8,8),y2=y1+m*dx;return adaptiveQ(sub,`A(${x1}, ${y1}), B(${x2}, ${y2}). Find the gradient.`,`m = (${y2} − ${y1}) / (${x2} − ${x1})<br>= ${y2-y1}/${dx}<br>= ${m}`,[`m = Δx/Δy<br>= ${dx}/${y2-y1}<br>= ${fmt(1/m)}`,`m = (${y2}+${y1}) / (${x2}+${x1})<br>= ${y2+y1}/${x2+x1}<br>= ${fmt((y2+y1)/(x2+x1||1))}`,`m = (${y2}−${x2}) / (${y1}−${x1})<br>= ${y2-x2}/${y1-x1}<br>= ${fmt((y2-x2)/(y1-x1||1))}`],'Gradient is change in y divided by change in x, with consistent point order.','GEO-GRAD',`GEO-GR-${x1}-${y1}-${x2}-${y2}`,sub,tier)}
 if(sub==='Straight-line equation'){const m=rnd(-5,6)||2,c=rnd(-9,9);if(tier<=2)return adaptiveQ(sub,`Gradient = ${m}, y-intercept = ${c}.`, `y = mx + c<br>y = ${m}x ${signed(c)}`,[`y = cx + m<br>y = ${c}x ${signed(m)}`,`x = my + c<br>x = ${m}y ${signed(c)}`,`Combine gradient and intercept.<br>y = (${m}${c>=0?'+':'−'}${Math.abs(c)})x<br>y = ${m+c}x`],'Use y = mx + c.','GEO-LINE-MC',`GEO-LM-${m}-${c}`,sub,tier);const x=rnd(-5,5),y=m*x+c;return adaptiveQ(sub,`Line has gradient ${m} and passes through (${x}, ${y}).`,`y − ${y} = ${m}(x − ${x})<br>y = ${m}x ${signed(c)}`,[`Use the point coordinates as m and c.<br>y = ${x}x ${signed(y)}<br>Check at x=${x}: y=${x*x+y}`,`Swap x- and y-coordinates in point-slope form.<br>y − ${x} = ${m}(x − ${y})<br>Then expand this equation.`,`Use y = mx + c but take c as the point's y-value.<br>y = ${m}x ${signed(y)}<br>Check at x=${x}: y=${m*x+y}`],'Use point-slope form or substitute the point into y = mx + c.','GEO-LINE-POINT',`GEO-LP-${m}-${x}-${y}`,sub,tier)}
 if(sub==='Distance between points'){const dx=rnd(2,12),dy=rnd(2,12),sq=dx*dx+dy*dy;return adaptiveQ(sub,`Horizontal change = ${dx}, vertical change = ${dy}. Find the distance.`,`d = √(${dx}² + ${dy}²)<br>= √${sq}${Number.isInteger(Math.sqrt(sq))?`<br>= ${Math.sqrt(sq)}`:''}`,[`d = Δx + Δy<br>= ${dx}+${dy}<br>= ${dx+dy}`,`d = √(Δx+Δy)<br>= √(${dx}+${dy})<br>= √${dx+dy}`,`d = √(Δx²−Δy²)<br>= √|${dx*dx}−${dy*dy}|<br>= √${Math.abs(dx*dx-dy*dy)}`],'Distance between points uses Pythagoras on horizontal and vertical changes.','GEO-DIST',`GEO-D-${dx}-${dy}`,sub,tier)}
 if(sub==='Area & perimeter'){const shape=['rectangle','triangle','trapezium'][Math.min(2,tier-1+rnd(0,1))];if(shape==='rectangle'){const l=rnd(4,18),w=rnd(3,14);return adaptiveQ(sub,`Rectangle: length ${l} cm, width ${w} cm. Find area.`,`A = l×w<br>= ${l}×${w}<br>= ${l*w} cm²`,[`A = 2(l+w)<br>= 2(${l}+${w})<br>= ${2*(l+w)} cm²`,`A = l+w<br>= ${l}+${w}<br>= ${l+w} cm²`,`A = l²+w²<br>= ${l}²+${w}²<br>= ${l*l+w*w} cm²`],'Area uses length × width; perimeter uses 2(l+w).','GEO-AREA-RECT',`GEO-AR-${l}-${w}`,sub,tier)}if(shape==='triangle'){const b=rnd(5,20),h=rnd(4,16);return adaptiveQ(sub,`Triangle: base ${b} cm, perpendicular height ${h} cm.`,`A = ½bh<br>= ½(${b})(${h})<br>= ${b*h/2} cm²`,[`A = bh<br>= ${b}×${h}<br>= ${b*h} cm²`,`A = ½(b+h)<br>= ½(${b}+${h})<br>= ${(b+h)/2} cm²`,`A = 2(b+h)<br>= 2(${b}+${h})<br>= ${2*(b+h)} cm²`],'Use the perpendicular height in A = ½bh.','GEO-AREA-TRI',`GEO-AT-${b}-${h}`,sub,tier)}const a=rnd(4,13),b=rnd(6,18),h=rnd(3,12);return adaptiveQ(sub,`Trapezium: parallel sides ${a} cm and ${b} cm, height ${h} cm.`,`A = ½(${a}+${b})(${h})<br>= ${(a+b)*h/2} cm²`,[`A = (a+b)h<br>= (${a}+${b})(${h})<br>= ${(a+b)*h} cm²`,`A = ½abh<br>= ½(${a})(${b})(${h})<br>= ${a*b*h/2} cm²`,`A = a+b+h<br>= ${a}+${b}+${h}<br>= ${a+b+h} cm²`],'Average the two parallel sides, then multiply by perpendicular height.','GEO-AREA-TRAP',`GEO-AZ-${a}-${b}-${h}`,sub,tier)}
...
function makeTrigV(tier,forced){const sub=forced||pickSubskill('trigonometry');
 if(sub==='Trig ratios'){const triples=[[3,4,5],[5,12,13],[8,15,17],[7,24,25]],T=triples[rnd(0,triples.length-1)],o=T[0],a=T[1],h=T[2],fn=['sin','cos','tan'][rnd(0,2)],correct=fn==='sin'?frac(o,h):fn==='cos'?frac(a,h):frac(o,a);return adaptiveQ(sub,`Right triangle relative to θ: opposite ${o}, adjacent ${a}, hypotenuse ${h}. Find ${fn} θ.`,`${fn} θ = ${fn==='sin'?'opposite/hypotenuse':fn==='cos'?'adjacent/hypotenuse':'opposite/adjacent'}<br>= ${correct}`,[`${fn} θ = ${fn==='sin'?'adjacent/hypotenuse':fn==='cos'?'opposite/hypotenuse':'opposite/hypotenuse'}<br>= ${fn==='sin'?frac(a,h):fn==='cos'?frac(o,h):frac(o,h)}`,`${fn} θ = ${fn==='sin'?'opposite/adjacent':fn==='cos'?'adjacent/opposite':'adjacent/hypotenuse'}<br>= ${fn==='sin'?frac(o,a):fn==='cos'?frac(a,o):frac(a,h)}`,`${fn} θ = ${fn==='sin'?'hypotenuse/opposite':fn==='cos'?'hypotenuse/adjacent':'hypotenuse/opposite'}<br>= ${fn==='sin'?frac(h,o):fn==='cos'?frac(h,a):frac(h,o)}`],'Match the requested ratio to SOH–CAH–TOA.','TRIG-RATIO',`TRIG-R-${fn}-${o}-${a}-${h}`,sub,tier)}
 if(sub==='Pythagoras'){const a=rnd(3,15),b=rnd(4,18),sq=a*a+b*b;return adaptiveQ(sub,`Right triangle legs are ${a} cm and ${b} cm. Find the hypotenuse.`,`c² = ${a}² + ${b}²<br>c = √${sq}${Number.isInteger(Math.sqrt(sq))?` = ${Math.sqrt(sq)}`:''}`,[`c² = a²+b²<br>c = ${a}+${b}<br>= ${a+b}`,`c² = a²−b²<br>c = √|${a*a}−${b*b}|<br>= √${Math.abs(a*a-b*b)}`,`c² = a+b<br>= ${a}+${b}<br>c = √${a+b}`],'The hypotenuse is opposite the right angle and satisfies c²=a²+b².','TRIG-PYTH',`TRIG-P-${a}-${b}`,sub,tier)}
 if(sub==='Sine rule'){const A=rnd(25,70),a=rnd(5,18),B=rnd(25,80);return adaptiveQ(sub,`In triangle ABC, a=${a}, A=${A}°, B=${B}°. Which setup correctly finds b?`,`b/sin ${B}° = ${a}/sin ${A}°<br>b = ${a}·sin ${B}°/sin ${A}°`,[`b/sin ${A}° = ${a}/sin ${B}°<br>b = ${a}·sin ${A}°/sin ${B}°`,`a/sin ${B}° = b/sin ${A}°<br>b = ${a}·sin ${A}°/sin ${B}°`,`a/cos ${A}° = b/cos ${B}°<br>b = ${a}·cos ${B}°/cos ${A}°`],'Pair each side with its opposite angle in the sine rule.','TRIG-SINE',`TRIG-S-${a}-${A}-${B}`,sub,tier)}
 if(sub==='Cosine rule'){const a=rnd(5,14),b=rnd(5,14),C=rnd(35,120);return adaptiveQ(sub,`Two sides are ${a} cm and ${b} cm with included angle ${C}°. Find opposite side c.`,`c² = ${a}² + ${b}² − 2(${a})(${b})cos ${C}°`,[`c² = a²+b²+2ab cos C<br>= ${a}²+${b}²+2(${a})(${b})cos ${C}°`,`c = a+b−2cos C<br>= ${a}+${b}−2cos ${C}°`,`c² = a²−b²−2ab cos C<br>= ${a}²−${b}²−2(${a})(${b})cos ${C}°`],'For two sides and the included angle, use c²=a²+b²−2ab cos C.','TRIG-COS',`TRIG-C-${a}-${b}-${C}`,sub,tier)}
 const a=rnd(5,16),b=rnd(6,18),C=rnd(25,135);return adaptiveQ('Triangle area',`Two sides are ${a} cm and ${b} cm with included angle ${C}°.`,`A = ½ab sin C<br>= ½(${a})(${b})sin ${C}°`,[`A = ab sin C<br>= (${a})(${b})sin ${C}°`,`A = ½ab cos C<br>= ½(${a})(${b})cos ${C}°`,`A = ½(a+b)sin C<br>= ½(${a}+${b})sin ${C}°`],'For a non-right triangle with two sides and included angle, use ½ab sin C.','TRIG-AREA',`TRIG-A-${a}-${b}-${C}`,sub,tier)
}
...
function makeStatsV(tier,forced){const sub=forced||pickSubskill('statistics');
 if(sub==='Mean / median / mode'){const vals=Array.from({length:tier>=3?7:5},()=>rnd(2,18));if(Math.random()<.5){const sum=vals.reduce((a,b)=>a+b,0);return adaptiveQ(sub,`Data: ${vals.join(', ')}. Find the mean.`,`Mean = (${vals.join(' + ')})/${vals.length}<br>= ${fmt(sum/vals.length)}`,[`Mean = total ÷ (n−1)<br>= ${sum}/${vals.length-1}<br>= ${fmt(sum/(vals.length-1))}`,`Order the data and take the middle value.<br>Median = ${median(vals)}<br>Use ${median(vals)} as the mean.`,`Mean = largest − smallest<br>= ${Math.max(...vals)}−${Math.min(...vals)}<br>= ${Math.max(...vals)-Math.min(...vals)}`],'Mean is total of all values divided by number of values.','STAT-MEAN',`STAT-M-${vals.join('-')}`,sub,tier)}const med=median(vals);return adaptiveQ(sub,`Data: ${vals.join(', ')}. Find the median.`,`Order the data first.<br>Middle value = ${med}`,[`Median = ${fmt(vals.reduce((a,b)=>a+b,0)/vals.length)}`,`Median = ${Math.max(...vals)-Math.min(...vals)}`,`Median = ${vals[0]}`],'Median is the middle value after ordering the data.','STAT-MED',`STAT-D-${vals.join('-')}`,sub,tier)}
 if(sub==='Probability'){const red=rnd(2,12),blue=rnd(2,12),green=tier>=3?rnd(1,8):0,total=red+blue+green;return adaptiveQ(sub,`Bag: ${red} red, ${blue} blue${green?`, ${green} green`:''}. Find P(red).`,`P(red) = ${red}/${total}<br>= ${frac(red,total)}`,[`P(red) = red/blue<br>= ${red}/${blue}<br>= ${frac(red,blue)}`,`P(red) = total/red<br>= ${total}/${red}<br>= ${frac(total,red)}`,`P(red) = blue/total<br>= ${blue}/${total}<br>= ${frac(blue,total)}`],'Probability = favourable outcomes ÷ total equally likely outcomes.','STAT-PROB',`STAT-P-${red}-${blue}-${green}`,sub,tier)}
 if(sub==='Quartiles & IQR'){let vals=Array.from({length:9},()=>rnd(1,30)).sort((a,b)=>a-b);const [q1,med,q3]=quartiles(vals),iqr=q3-q1;return adaptiveQ(sub,`Ordered data: ${vals.join(', ')}. Find the IQR.`,`Q1 = ${q1}, Q3 = ${q3}<br>IQR = ${q3} − ${q1} = ${iqr}`,[`IQR = maximum−minimum<br>= ${Math.max(...vals)}−${Math.min(...vals)}<br>= ${Math.max(...vals)-Math.min(...vals)}`,`IQR = Q1+Q3<br>= ${q1}+${q3}<br>= ${q1+q3}`,`IQR = median<br>= ${med}<br>= ${med}`],'IQR is upper quartile minus lower quartile.','STAT-IQR',`STAT-I-${vals.join('-')}`,sub,tier)}
 if(sub==='Standard deviation'){const vals=Array.from({length:5},()=>rnd(1,12)),mu=vals.reduce((a,b)=>a+b,0)/vals.length,variance=vals.reduce((a,b)=>a+(b-mu)**2,0)/vals.length,sd=Math.sqrt(variance);return adaptiveQ(sub,`Population data: ${vals.join(', ')}. Which setup is correct for σ?`,`μ = ${fmt(mu)}<br>σ = √[Σ(x−μ)²/${vals.length}]<br>≈ ${fmt(sd)}`,[`μ = ${fmt(mu)}<br>σ = Σ|x−μ|/${vals.length}<br>≈ ${fmt(vals.reduce((a,b)=>a+Math.abs(b-mu),0)/vals.length)}`,`μ = ${fmt(mu)}<br>σ = √[Σ(x−μ)²/${Math.max(1,vals.length-1)}]<br>≈ ${fmt(Math.sqrt(vals.reduce((a,b)=>a+(b-mu)**2,0)/Math.max(1,vals.length-1)))}`,`μ = ${fmt(mu)}<br>σ = Σ(x−μ)²/${vals.length}<br>≈ ${fmt(variance)}`],'Population standard deviation uses squared deviations from the mean, averaged, then square-rooted.','STAT-SD',`STAT-SD-${vals.join('-')}`,sub,tier)}
 const cats=['A','B','C','D'],freq=cats.map(()=>rnd(2,15)),total=freq.reduce((a,b)=>a+b,0),idx=rnd(0,3),angle=360*freq[idx]/total;const table=`<table class="mini-data"><tr>${cats.map(c=>`<th>${c}</th>`).join('')}</tr><tr>${freq.map(f=>`<td>${f}</td>`).join('')}</tr></table>`;return adaptiveQ('Data displays',`${table}For a pie chart, find the sector angle for ${cats[idx]}.`,`Angle = ${freq[idx]}/${total} × 360°<br>= ${fmt(angle)}°`,[`Angle = frequency×360°<br>= ${freq[idx]}×360°<br>= ${freq[idx]*360}°`,`Angle = total/frequency×360°<br>= ${total}/${freq[idx]}×360°<br>= ${fmt(total/freq[idx]*360)}°`,`Angle = frequency/total×100°<br>= ${freq[idx]}/${total}×100°<br>= ${fmt(freq[idx]/total*100)}°`],'Pie-chart sector angle is category frequency divided by total frequency, multiplied by 360°.','STAT-PIE',`STAT-PIE-${freq.join('-')}-${idx}`,sub,tier)
}
...
function joinSharedBattle(sessionId,data){
 if(!data||data.status==='ended'&&!data.result)return;
 if(battle?.sessionId===sessionId&&!battle.ended){syncSharedBattle(sessionId,data);return}
 const me=window.MathClansOnline?.user?.();if(!me)return;
 const side=data.sides?.attacker?.participants?.[me.uid]?'attacker':data.sides?.defender?.participants?.[me.uid]?'defender':null;if(!side)return;
 const enemySide=side==='attacker'?'defender':'attacker',sideData=data.sides?.[side]||{},enemyData=data.sides?.[enemySide]||{};
 const team=Object.keys(sideData.participants||{}).map(uid=>{const local=members.find(m=>m.uid===uid),remote=data.participants?.[uid]||sideData.participants?.[uid]||{};return local||{uid,name:remote.name||'Mathling',avatar:remote.avatar||'🐲',status:'deployed',skills:remote.skills||{algebra:1,geometry:1,trigonometry:1,statistics:1}}});
 if(!team.length)return;
 const enemy={name:enemyData.clanName||'Rival Clan',crest:enemyData.guardian||'🐉',region:enemyData.region||'—',rating:Number(enemyData.rating||1500),skills:sideAverageSkills(enemyData.participants||{})};
 const selectedIdx=team.map(m=>members.findIndex(x=>x.uid&&m.uid&&x.uid===m.uid)).filter(i=>i>=0);selectedIdx.forEach(i=>members[i].status='deployed');renderMembers();renderPresence();
 beginBattleLocal(enemy,team,selectedIdx,sessionId,{...data,formation:sideData.formation||'balanced',mySide:side,enemySide});syncSharedBattle(sessionId,data);
}
function sideAverageSkills(participants){const arr=Object.values(participants||{}),out={algebra:1,geometry:1,trigonometry:1,statistics:1};if(!arr.length)return out;for(const k of Object.keys(out))out[k]=Math.round(arr.reduce((a,m)=>a+Number(m?.skills?.[k]||1),0)/arr.length);return out}
function beginBattleLocal(enemy,team,selectedIdx,sessionId,sessionData){
 const n=team.length,avg=k=>Math.round(team.reduce((a,m)=>a+Number(m.skills?.[k]||1),0)/Math.max(1,n));
 battle={enemy,team,selectedIdx,formation:sessionData?.formation||selectedFormation,maxHp:10000,ourHp:10000,enemyHp:10000,seconds:Math.max(1,Math.ceil(((sessionData?.endsAt||Date.now()+90000)-Date.now())/1000)),asked:0,correct:0,combo:0,started:sessionData?.startedAt||Date.now(),question:null,lastQAt:Date.now(),teamPowerSamples:[],forfeitUids:new Set(),skills:Object.fromEntries(Object.keys(SKILLS).map(k=>[k,avg(k)])),sessionId:sessionId||null,sharedParticipants:sessionData?.participants||null,mySide:sessionData?.mySide||null,enemySide:sessionData?.enemySide||null,sharedEndsAt:Number(sessionData?.endsAt||0),sharedResult:null};
 setMusicMode('battle');window.MathClansOnline?.setActivity?.('battle');
 $('#battleOurClan').textContent=sessionData?.sides?.[battle.mySide]?.clanName||state.clan.name;$('#battleEnemyClan').textContent=enemy.name;$('#ourFormation').innerHTML=team.map(m=>`<span class="unit">${m.avatar}</span>`).join('');const enemyTeam=Object.values(sessionData?.sides?.[battle.enemySide]?.participants||{});$('#enemyFormation').innerHTML=(enemyTeam.length?enemyTeam:team).map((m,i)=>`<span class="unit">${m.avatar||['🐙','🐺','🦇','🐗','🦂','🦅','🐯','🦈','🐍','🦁'][i]}</span>`).join('');$('#battleTeamSize').textContent=sessionData?.sides?`${team.length}v${enemyTeam.length}`:`${n}v${n}`;const bf=$('#battleFormation');if(bf)bf.textContent=`${FORMATIONS[battle.formation]?.icon||'⚖'} ${FORMATIONS[battle.formation]?.name||'Balanced'}`;$('#battleSkillRunes').innerHTML=Object.entries(SKILLS).map(([k,s])=>`<div class="rune">${s.icon} ${s.name}<b>${battle.skills[k]}</b></div>`).join('');goScreen('battle');nextMental();updateBattleUi();$('#mentalAnswer').value='';$('#mentalAnswer').focus();battle.timer=setInterval(tickBattle,1000);
}
...
function submitMental(){
 if(!battle||battle.ended)return;const val=Number($('#mentalAnswer').value);if(!Number.isFinite(val))return;const elapsed=(Date.now()-battle.lastQAt)/1000;battle.asked++;const playerCorrect=val===battle.question.ans;let playerScore=0;
 if(playerCorrect){battle.correct++;battle.combo++;playerScore=Math.round(100+Math.max(0,40-(elapsed-1.5)*10));playSfx('correct')}else{battle.combo=0;playSfx('wrong')}battle.localScoreTotal=Number(battle.localScoreTotal||0)+playerScore;
 if(battle.sessionId){window.MathClansClans?.reportBattleScore?.(battle.sessionId,playerScore,playerCorrect,elapsed).catch?.(()=>{});effect(playerCorrect?`⚔ ${playerScore} battle points`:'🛡 Miss — keep going');updateBattleUi();nextMental();$('#mentalAnswer').focus();return}
 const teamScores=[playerScore,...battle.team.slice(1).map(teammateWaveScore)],teamAvg=teamScores.reduce((a,b)=>a+b,0)/Math.max(1,teamScores.length);battle.teamPowerSamples.push(teamAvg);if(battle.teamPowerSamples.length>30)battle.teamPowerSamples.shift();const troopBonus=1+Math.min(.05,(battle.team.length-1)*.005),fm=FORMATIONS[battle.formation]?.mods||FORMATIONS.balanced.mods,alg=1+softCap(battle.skills.algebra*fm.algebra,.10,95),trig=playerCorrect&&elapsed<3?1+softCap(battle.skills.trigonometry*fm.trigonometry,.06,110):1;let tactical=1;if(Math.random()<Math.min(.05,softCap(battle.skills.statistics*fm.statistics,.05,120))){tactical=1.35;effect('📊 CRITICAL OUTLIER!');playSfx('critical')}const enemyBlock=1-softCap(battle.enemy.skills.geometry,.10,100);if(playerCorrect||teamAvg>70){const power=(teamAvg/140)*760*troopBonus*alg*trig*tactical,dmg=Math.max(1,Math.round(power*enemyBlock));battle.enemyHp=Math.max(0,battle.enemyHp-dmg);if(tactical===1)effect(`⚔ TEAM STRIKE ${dmg}`)}else{const enemyWave=95+Math.random()*25,ourBlock=1-softCap(battle.skills.geometry*fm.geometry,.10,100),enemyDmg=Math.round((enemyWave/140)*620*ourBlock);battle.ourHp=Math.max(0,battle.ourHp-enemyDmg);effect(`🛡 ENEMY COUNTER ${enemyDmg}`);playSfx('shield')}updateBattleUi();if(battle.enemyHp<=0||battle.ourHp<=0)return endBattle();nextMental();$('#mentalAnswer').focus()
}
function tickBattle(){if(!battle||battle.ended)return;if(battle.sessionId){battle.seconds=Math.max(0,Math.ceil(((battle.sharedEndsAt||Date.now()+1000)-Date.now())/1000));$('#battleTimer').textContent=battle.seconds;if(battle.seconds<=0)window.MathClansClans?.finishBattleForPlayer?.(battle.sessionId).catch?.(()=>{});return}battle.seconds--;$('#battleTimer').textContent=battle.seconds;if(battle.seconds<=0)endBattle();else if(Math.random()<.22){const dmg=Math.round(180*(1-battle.skills.geometry/1500));battle.ourHp=Math.max(0,battle.ourHp-dmg);updateBattleUi();if(battle.ourHp<=0)endBattle()}}
function updateBattleUi(){if(!battle)return;$('#ourHpBar').style.width=(battle.ourHp/battle.maxHp*100)+'%';$('#enemyHpBar').style.width=(battle.enemyHp/battle.maxHp*100)+'%';$('#ourHpText').textContent=`${Math.round(battle.ourHp)} HP`;$('#enemyHpText').textContent=`${Math.round(battle.enemyHp)} HP`;$('#battleAccuracy').textContent=(battle.asked?Math.round(battle.correct/battle.asked*100):100)+'%';$('#battleCombo').textContent=battle.combo;const a=battle.sharedTeamAvg!=null?Math.round(battle.sharedTeamAvg):(battle.teamPowerSamples?.length?Math.round(battle.teamPowerSamples.reduce((x,y)=>x+y,0)/battle.teamPowerSamples.length):0);$('#battleTeamAvg').textContent=a;const ma=$('#battleMyAvg');if(ma)ma.textContent=battle.asked?Math.round((battle.localScoreTotal||0)/battle.asked):0;const oa=$('#battleOpponentAvg');if(oa)oa.textContent=Math.round(Number(battle.sharedOpponentAvg||0))}
function syncSharedBattle(sessionId,data){if(!battle||battle.sessionId!==sessionId||!data)return;battle.sharedParticipants=data.participants||{};battle.sharedEndsAt=Number(data.endsAt||battle.sharedEndsAt||Date.now());const my=battle.mySide||'attacker',enemy=battle.enemySide||'defender',sb=data.scoreboard||{};battle.sharedTeamAvg=Number(sb[my]?.teamAvg||0);battle.sharedOpponentAvg=Number(sb[enemy]?.teamAvg||0);battle.ourHp=Number(sb[my]?.hp??battle.ourHp);battle.enemyHp=Number(sb[enemy]?.hp??battle.enemyHp);battle.seconds=Math.max(0,Math.ceil((battle.sharedEndsAt-Date.now())/1000));$('#battleTimer').textContent=battle.seconds;updateBattleUi();if(data.result&&!battle.sharedResult){battle.sharedResult=data.result;endSharedBattle(data.result,data)}}
function effect(t){$('#battleEffects').innerHTML=`<div class="effect-text">${t}</div>`}
function endSharedBattle(result,data){if(!battle||battle.ended)return;battle.ended=true;clearInterval(battle.timer);window.MathClansOnline?.setActivity?.('online');(battle.selectedIdx||[]).forEach(i=>{if(members[i])members[i].status='ready'});selectedMembers.clear();renderMembers();renderPresence();const mySide=battle.mySide||'attacker',won=result.winner===mySide,draw=result.winner==='draw',delta=Number(result.ratingDelta?.[mySide]||0),myStats=result.sides?.[mySide]||{},enemyStats=result.sides?.[battle.enemySide]||{};if(won)state.player.crystals+=80;state.clan.rating=Math.max(1000,Number(state.clan.rating||1500)+delta);if(won)state.clan.influence=Math.min(100,Number(state.clan.influence||0)+2);save();renderRanking();const cr=$('#clanRating');if(cr)cr.textContent=state.clan.rating;effect(draw?'⚖ DRAW':won?'🏆 VICTORY!':'💥 DEFEAT');playSfx(draw?'shield':won?'victory':'defeat');setMusicMode('ambient');showModal(`<span class="eyebrow">V2.0 LIVE CLAN CLASH RESULT</span><h3>${draw?'⚖ Draw':won?'🏆 Victory':'🛡 Defeat'}</h3><p>${data?.sides?.[mySide]?.clanName||state.clan.name} vs ${data?.sides?.[battle.enemySide]?.clanName||battle.enemy.name}</p><div class="clan-stats"><div><span>Team Avg</span><b>${Math.round(Number(myStats.teamAvg||0))}</b></div><div><span>Accuracy</span><b>${Math.round(Number(myStats.accuracy||0))}%</b></div><div><span>Final Power</span><b>${Math.round(Number(myStats.finalPower||0))}</b></div><div><span>Opponent Avg</span><b>${Math.round(Number(enemyStats.teamAvg||0))}</b></div><div><span>Rating</span><b>${delta>0?'+':''}${delta}</b></div><div><span>Crystals</span><b>${won?'+80':'0'}</b></div></div><p>Every deployed player is included in the team average. A player who leaves during battle counts as <b>0 points</b>.</p><div class="modal-actions"><button class="primary" id="returnMap">Return to Map</button></div>`);$('#returnMap').onclick=()=>{closeModal();battle=null;goScreen('map');init()}}
function endBattle(){if(!battle||battle.ended)return;if(battle.sessionId){window.MathClansClans?.finishBattleForPlayer?.(battle.sessionId).catch?.(()=>{});return}battle.ended=true;clearInterval(battle.timer);window.MathClansOnline?.setActivity?.('online');(battle.selectedIdx||[]).forEach(i=>members[i].status='ready');selectedMembers.clear();renderMembers();renderPresence();const win=battle.enemyHp<battle.ourHp,delta=win?rnd(18,28):-rnd(12,20);state.clan.rating=Math.max(1000,state.clan.rating+delta);if(win){state.player.crystals+=80;state.clan.influence=Math.min(100,state.clan.influence+2)}save();renderRanking();$('#clanRating').textContent=state.clan.rating;effect(win?'🏆 VICTORY!':'💥 DEFEAT');playSfx(win?'victory':'defeat');setMusicMode('ambient');showModal(`<span class="eyebrow">CLAN CLASH RESULT</span><h3>${win?'🏆 Victory':'🛡 Defeat'}</h3><p>${state.clan.name} vs ${battle.enemy.name}</p><div class="modal-actions"><button class="primary" id="returnMap">Return to Map</button></div>`);$('#returnMap').onclick=()=>{closeModal();battle=null;goScreen('map');init()}}

// ===== V1.9 live multiplayer battle sync =====// ===== V1.8 real clan war lobby + deployment =====
...
function openWarCouncil(i){
 ...
 showModal(`<span class="eyebrow">V2.0 WAR LOBBY · STEP 2</span><h3>${state.clan.guardian} ${state.clan.name} → ${enemy.crest||enemy.guardian||'🐉'} ${enemy.name}</h3><p>Choose a formation, then rally your selected members. If they accept, a live challenge is sent to ${enemy.name}.</p><div class="war-team-preview">${team.map(m=>`<div><span>${m.avatar}</span><small>${m.name}</small></div>`).join('')}</div><div class="war-stat-grid">${Object.entries(SKILLS).map(([k,s])=>`<div><span>${s.icon} ${s.name}</span><b>${avg(k)}</b></div>`).join('')}</div><div class="formation-picker">${Object.entries(FORMATIONS).map(([k,f])=>`<button type="button" class="formation-card ${selectedFormation===k?'selected':''}" data-formation="${k}"><span>${f.icon}</span><strong>${f.name}</strong><small>${f.desc}</small></button>`).join('')}</div><div class="modal-actions"><button type="button" class="secondary" id="warBack">Back</button><button type="button" class="primary" id="confirmMarch">Rally ${team.length} Member${team.length>1?'s':''}</button></div>`);
